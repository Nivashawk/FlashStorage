import { Component, OnInit } from '@angular/core';
import {FormGroup, FormArray, FormBuilder, Validators, FormControl, AbstractControl} from '@angular/forms';
import * as pdfMake from "pdfmake/build/pdfmake";
import * as pdfFonts from 'pdfmake/build/vfs_fonts';
(<any>pdfMake).vfs = pdfFonts.pdfMake.vfs;
import dit_form_data from "../../../data/forms/dit.json"



@Component({
  selector: 'app-dit',
  templateUrl: './dit.component.html',
  styleUrls: ['./dit.component.css']
})
export class DitComponent implements OnInit {

  dit_data = dit_form_data
  public first_entry:FormGroup;
  results:object
  formcontrol:any;
  data
  TND:number;
  PC:number;
  TNSC:number;
  SC:number;
  SCID:number;
  HST:number;
  HDD_PRICE:number
  OVERALL_HDD_PRICE:any
  TOTAL_STORAGE_NEEDED:number
  HDD_NEEDED:any
  TOTAL_HDD_NEEDED:any
  OVERALL_SERVICE_CHARGES:any
  QUOTATION_WITHOUT_GST:number
  QUOTATION_WITH_GST:any
  GST:number
  HDD_TYPE
  primary_camera_budget
  secondary_camera_budget
  PRO_NAME
  checkbox_flag:FormControl = new FormControl(false);
  submitError: Boolean = false;

  constructor(public fb: FormBuilder) { }

  hdd_price(HST):any
  {
    if(HST == 3.5)
    {
      return 8600;
    }
    else if (HST == 5.4)
    {
      return 13000;
    }
    else if (HST == 7.2)
    {
      return 17000;
    }
  }

  hdd_variant(hdd_price):any
  {
    if(hdd_price == 8600)
    {
      return 4;
    }
    else if (hdd_price == 13000)
    {
      return 6;
    }
    else if (hdd_price == 17000)
    {
      return 8;
    }

  }

  onSubmit(){
    // console.log("submit clicked", this.first_entry.invalid);
    this.PRO_NAME = this.first_entry.get('PRODUCTION_NAME').value
    // console.log(this.TND);
    this.TND = this.first_entry.get('TOTAL_NO_OF_DAYS').value
    // console.log(this.TND);
    this.PC = this.first_entry.get('SELECT_THE_PRIMARY_CAMERA').value
    // console.log(this.PC);
    this.TNSC = this.first_entry.get('TOTAL_NO_OF_SECONDARY_CAMERA').value
    // console.log(this.TNSC);
    this.SC = this.first_entry.get('SELECT_THE_SECONDARY_CAMERA').value
    // console.log(this.SC);
    this.SCID = this.first_entry.get('USAGE_OF_SECONDARY_CAMERA_IN_DAYS').value
    // console.log(this.SCID);
    this.HST = this.first_entry.get('HDD_STORAGE_TYPE').value
    // console.log(this.HST);

    // QOUTATION CALCULATION STARTS HERE

    //calculates the primary camera cost by multipling total days and camera type
    this.primary_camera_budget = this.TND * this.PC
    // console.log(this.primary_camera_budget);

    //calculates the secondary camera cost by multipling total days, camera type and no of cameras used
    this.secondary_camera_budget = this.SC * this.SCID
    // console.log(this.secondary_camera_budget);
    
    //calculates the total estimated storage needed by adding primary camera storage and secondary camera storage space
    this.TOTAL_STORAGE_NEEDED = (this.primary_camera_budget + this.secondary_camera_budget)
    // console.log(this.TOTAL_STORAGE_NEEDED);

    //calculates the total no of harddisk need for the project by dividing total storage needed and harddiks type
    this.HDD_NEEDED = (this.TOTAL_STORAGE_NEEDED / this.HST)
    // console.log(this.HDD_NEEDED);

    this.TOTAL_HDD_NEEDED = (this.HDD_NEEDED * 2)
    // console.log(this.TOTAL_HDD_NEEDED);
    
    this.HDD_PRICE = this.hdd_price(this.HST)
    // console.log(this.HDD_PRICE);

    this.HDD_TYPE = this.hdd_variant(this.HDD_PRICE)
    
    this.OVERALL_HDD_PRICE = (this.HDD_PRICE * this.TOTAL_HDD_NEEDED)
    // console.log(this.OVERALL_HDD_PRICE);
    
    this.OVERALL_SERVICE_CHARGES = (this.TND * 3000)
    // console.log(this.OVERALL_SERVICE_CHARGES);

    this.QUOTATION_WITHOUT_GST = Math.round((this.OVERALL_HDD_PRICE + this.OVERALL_SERVICE_CHARGES))
    // console.log(this.QUOTATION_WITHOUT_GST);

    this.GST = Math.round(this.QUOTATION_WITHOUT_GST * 0.18)
    
    this.QUOTATION_WITH_GST = Math.round((this.QUOTATION_WITHOUT_GST + this.GST)).toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,");
    


    // QOUTATION CALCULATION ENDS HERE


    // PDF GENERATOR STARTS HERE

    var dd = {
      pageSize: 'A4',
      pageOrientation: 'portrait',
      watermark: { text: 'FLASH STORAGE', angle: -30, opacity: 0.1, bold: true, fontSize: 70},
      info: {
        title: 'FLASH STORAGE DIT QOUTATION',
        author: 'FLASH STORAGE',
        },
      content: [

        {
          image: 'data:image/png;base64,',
          height: 40,
          width: 80,
          style: 'img'
        },

        // HEADING

        {
          text: 'DIGITAL DATA MANAGEMENT',
          style: 'header',
          alignment: 'center',
        },

        // HEADING ENDS
      
        // BASIC INFO

        {
          alignment: 'justify',
          style:'to',
          columns: [
            {
              text: 'To,'
            },
            {
              width: 90,
              text: `Date:${new Date().toLocaleDateString()}`
            }
          ],
        },
        {
          text: `Production:${this.PRO_NAME}`,
          style: 'sub'
        },

        // BASIC INFO ENDS

        // SUB STARTS

        {
          text: 'Sub:Digital Film Data Management Services',
          style: 'sub',
          alignment: 'center',
        },

        // // SUB ENDS

        // RESPECTED SIR STARTS

        {
          text: 'Respected Sir, \n\n With reference to your enquiry regarding Digital Data Management Services, we are pleased to quote our PACKAGE as follows;',
          style: 'para',
          alignment: 'left',
        },
        
        // RESPECTED SIR ENDS

        // QUOTATION STARTS

        {
          text: 'QUOTATION:',
          style: 'subheading1',
          alignment: 'left',
        },

        {
          text: 'Scope Of Work:',
          style: 'subheading2',
          alignment: 'left',
        },

        {
          text: 'Off Set/On Set: Transfer & Transcoding of SOURCE data for offline editing.',
          style: 'para',
          alignment: 'left',
        },

        {
          text: 'Tariff:',
          style: 'subheading2',
          alignment: 'left',
        },

        {
          text: 'Transfer and backup of RAW/SOURCE footage',
          style: 'para',
          alignment: 'left',
        },

        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: 'ESTIMATED TOTAL NO. OF DAYS OF SHOOT'
            },
            {
              text: `${this.TND} days`
            }
          ],
        },

        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: 'ESTIMATED STORAGE NEEDED PER DAY'
            },
            {
              text: `${this.PC} TB`
            }
          ],
        },

        {
          style:'para',
          columns: [
            {
              text: 'ESTIMATED TOTAL STORAGE NEEDED'
            },
            {
              text: `${Math.round(this.TOTAL_STORAGE_NEEDED)} TB`
            }
          ],
        },

        {
          style:'para',
          columns: [
            {
              text: 'HDD VARIANT'
            },
            {
              text: `${this.HDD_TYPE}TB - (${this.HST} TB Actual Space)`
            }
          ],
        },

        {
          style:'para',
          columns: [
            {
              text: 'ESTIMATED TOTAL HDD NEEDED'
            },
            {
              text: `${Math.round(this.TOTAL_HDD_NEEDED)} (includes production backup)`
            }
          ],
        },

        {
          style:'para',
          columns: [
            {
              text: 'ESTIMATED TOTAL HDD COST'
            },
            {
              text: `RS.${Math.round(this.OVERALL_HDD_PRICE)}/- `
            }
          ],
        },

        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: 'ESTIMATED SERVICE CHARGES'
            },
            {
              text: `Rs.${this.OVERALL_SERVICE_CHARGES}/- `
            }
          ],
        },

        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: 'ESTIMATED OVERALL COST WITHOUT GST'
            },
            {
              color:'red',
              text: `RS.${this.QUOTATION_WITHOUT_GST}/- `
            }
          ],
        },

        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: '18% GST OF OVERALL COST'
            },
            {
              color:'red',
              text: `RS.${this.GST}/-`
            }
          ],
        },

        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: 'ESTIMATED OVERALL COST WITH GST'
            },
            {
              color:'red',
              text: `RS.${this.QUOTATION_WITH_GST}/- ONLY`
            }
          ],
        },

        
        {
          text: 'Terms:',
          style: 'subheading2',
          alignment: 'left',
        },

        {
          text: '1. FLASH STORAGE will be responsible for maintaining Three Backup copies and Transcode files in their own Server Hard drive and Raid Box.',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '2. From the storage we maintain everything in our office.',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '3. Flash storage will take care of the required HDD’s for Transportation of the footage for (Editing/CG) purpose.',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '4. No extra charges for multiple cameras.',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '5. Daily conveyance Rs.500/-(For the D.I.Technician in the shooting spot)',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '6. Editing Footage will be followed by the Flash Storage and team',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '7. After the completion of the shoot single Hard disk Backup will be given to Producer or D.I. After Settlement of the full amount, you will receive the single backup RAW files (Hard Disk).',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '8. We keep overall footage for 6 months from the end of the last shoot day. We will inform the production company before erasing the footage from the server after the grace period as mentioned. If incase the production wants a backup, Production must provide us the required Hard Disks. We will provide the entire footage in Hard drive.',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '9. This kind of backup is 100 percent safe and secure. Kindly do the agreement form and other formalities.',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '10. Daily Transport & A.C Rooms or Caravans must be provided for the D.I.T in the shooting spot. On the outdoor shoot the D.I.T’s must be provided with A.C rooms. It is compulsory to ensure the safety of the footages.',
          style: 'para',
          alignment: 'left',
        },

        {
          text: '11. 50% Advance payment must be paid before the shoot starts,Balance amount must be paid before the shoot ends.\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n',
          style: 'para',
          width:40,
          alignment: 'left',
        },

        {
          text: 'Sincerely',
          style: 'para',
          width:40,
          alignment: 'left',
        },

        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: 'For FLASH STORAGE'
            },
            {
              text: 'Herby accept & confirm the above'
            }
          ],
          columnGap: 200
        },


        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: 'Authorized Signatory'
            },
            {
              text: 'The Producer'
            }
          ],
          columnGap: 200
        },

        {
          alignment: 'justify',
          style:'para',
          columns: [
            {
              text: ''
            },
            {
              text: 'DATE'
            }
          ],
          columnGap: 200
        },

        

        
        
        // QUOTATION ENDS

      ],

      // STYLING STARTS

      styles: {

        img:{
          margin: [0,10, 0, 20]
        },

        header: {
          fontSize: 18,
          bold: true,
          alignment: 'justify',
          margin: [0, 10, 0, 30]
        },

        to: {
          fontSize: 10,
          alignment: 'justify',
          margin: [0, 10, 0, 10]
        },

        sub: {
          fontSize: 10,
          decoration: 'underline',
          bold: true,
          alignment: 'justify',
          margin: [0, 10, 0, 10]
        },

        subheading1: {
          fontSize: 10,
          bold: true,
          decoration: 'underline',
          alignment: 'justify',
          margin: [0, 10, 0, 10]
        },

        subheading2: {
          fontSize: 10,
          bold: true,
          alignment: 'justify',
          margin: [0, 10, 0, 10]
        },

        para: {
          fontSize: 10,
          alignment: 'justify',
          margin: [0, 10, 0, 10]
        }
      }
    }


    if (!this.first_entry.invalid) {
      this.submitError = false;
      pdfMake.createPdf(dd).download("FLASHSTORAGE DIT-EXACT_QUOTATION");
    } else {
      this.submitError = true;
    }

    // PDF GENERATOR ENDS HERE

    




   
 
  }

  

  ngOnInit(): void {
   
    // console.log(this.checkbox_flag.value);
    
    

    this.data = this.dit_data
    this.first_entry = this.fb.group({
      PRODUCTION_NAME : new FormControl(),
      TOTAL_NO_OF_DAYS : new FormControl
      (
        '', 
        [
          Validators.required,
          Validators.pattern('[0-9]+')
        ]
        ),
      SELECT_THE_PRIMARY_CAMERA : new FormControl('', Validators.required),
      TOTAL_NO_OF_SECONDARY_CAMERA : new FormControl
      (
        '0', 
        [
          Validators.required,
          Validators.pattern('[0-9]+')
        ]
        ),
      SELECT_THE_SECONDARY_CAMERA : new FormControl(0, Validators.required),
      USAGE_OF_SECONDARY_CAMERA_IN_DAYS : new FormControl
      (
        '0', 
        [
          Validators.required,
          Validators.pattern('[0-9]+')
        ]
        ),
      HDD_STORAGE_TYPE : new FormControl('', Validators.required),
      // checkbox_value : new FormControl('', Validators.required)
    });


    // this.first_entry.controls.checkbox_value.valueChanges.subscribe((value)=>{
    //   console.log(value);
    //   this.checkbox_flag.setValue(this.first_entry.get("checkbox_value").value);
      
    //   if (!value) {
    //     console.log("inside this block!!!");
    //     this.first_entry.controls.SELECT_THE_SECONDARY_CAMERA.setValue('0');
    //     this.first_entry.controls.USAGE_OF_SECONDARY_CAMERA_IN_DAYS.setValue('0');
    //   }
    // })
  }
}
